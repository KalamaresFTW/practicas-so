...
char *searchlist[MAXSEARCHLIST];

void proc_cmd(char *params[]){
    if (params[0] == NULL) {
        printf("%s\n", "proc pid");
        return;
    } else {
        imprimir_LinkedList(atoi(params[0]));
    }
}
...
void background_cmd(char *params[], int numparams){
    ...
    for (int i = 0; i < numparams; i++) {
        if (params[i][0] == '@') {
            prioridad = atoi(params[i]+1);
            params[i] = NULL;
            numparams -= 1;
            if (prioridad != -9999) {
                setpriority(PRIO_PROCESS, 0, prioridad);
            }
        }
    }

    program = BuscarEjecutable(params[0]);
    pid = fork();
    if ((pid == 0) && (program != NULL)){
        if (program != NULL) {
            setsid();
            freopen("/dev/null", "r", stdin);
            freopen("/dev/null", "w", stdout);
            freopen("/dev/null", "x", stderr); //
        ...
        data.pid = pid;
        data.date = time(NULL);
        data.priority = getpriority(PRIO_PROCESS, pid);
        int i = 0;
        strcpy(data.command, "\0");
        while (params[i]!=NULL) {
            strcat(&data.command[i], params[i]);
            strcat(&data.command[i], " ");
            i++;
        }
        insertar_LinkedList(data);
    }
}

void exec_cmd(char *params[], int numparams){
    char *program = NULL;
    char *params_copia[numparams];
    int prioridad = -9999;
    ...
    if ((program = BuscarEjecutable(params[0])) != NULL) {
        params_copia[0] = program;
        for (int i = 1; i < numparams; i++) {
            params_copia[i] = params[i];
        }
        execv(program, params_copia);
        perror("execv");
    } else {
        printf("%s\n", "Program not found.");
    }
...
void searchlist_cmd(char *params[]){
    char *directorio = &params[0][1];
    char *exfile;
    if (params[0] == NULL) {
        SearchListShow();
    } else if(!strcmp(params[0],"-path")){
        SearchListAddPath();
    } else if(params[0][0] == '+'){
        SearchListAddDir(directorio);
    } else {
        exfile = BuscarEjecutable(params[0]);
        if (exfile != NULL) {
            printf("%s\n", exfile);
        } else {
            printf("No se ha encontado: %s\n", params[0]);
        }
    }
}
...
void setpriority_cmd(char *params[]){
    if ((params[0] != NULL) && (params[1] != NULL)) {
        setpriority(PRIO_PROCESS, atoi(params[0]), atoi(params[1]));
    } else if (params[0] != NULL) {
        printf("%d\n", getpriority(PRIO_PROCESS, atoi(params[0])));
    } else {
        printf("setpriority [pid] [value]\n");
    }
}
